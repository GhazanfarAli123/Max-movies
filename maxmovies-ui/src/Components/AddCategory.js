...
const AddCategory = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const { data } = await axios.post(
                'http://localhost:1000/api/v1/category/create-category',
                { name },
                {
                    headers: {
            ...
            console.log(`category ${data.name} added`);
            getCategory()
        } catch (err) {
            console.log(err);
        }
    ...
    const handelDelete = async (id) => {
        try {
            const { data } = await axios.delete(`http://localhost:1000/api/v1/category/delete-category/${id}`, {
                headers: {
                    'auth-token': auth.token
                }
            })
